...
export class AlphaSync {
  ...
  public async generate_tree(){
    if (this.contentDirectory){
      ...
      let totalLength = 0;
      for (let key in this.contentDirectory.date_to_items) {
...
public async get_single_image(save_path: string): Promise<void> {
  ...
  public async get_all_images_dict(save_path: string){
    for (let key in this.date_to_items) {
      console.log(`downloading for date ${key}`)
      let p = path.join(save_path,key)
      let files = new Set()
      if (fs.existsSync(p)) {
       
          fs.readdirSync(p).forEach((file) => {files.add(file)
          console.log(file)})
      }

      if (this.date_to_items.hasOwnProperty(key)) {
          let value = this.date_to_items[key];
